...
export class AppointmentMigration {
  ...
  createAppointments () {
    ...
    return new Promise((resolve, reject) => {
      connection.query(
        `CREATE TABLE IF NOT EXISTS appointments (${structure});`,
        function (err, result, fields) {
          if (err) {
            console.error('Error al crear la tabla de appointments:', err)
            reject(err)
          }
          console.log('Tabla appointments creada con éxito.')
          resolve(result)
        }
      )
      connection.end()
