...
let jwt = ''
let jwtCookie = ''
let messages = {} // El mensaje creado en el test de post para usarlo en el resto de tests
...
beforeAll(async () => {
  ...
  const response = xsrfTokenGet.header['set-cookie']
  const splitToken = response[0].split(';', 1)
  xsrfToken = splitToken[0].split('=', 2)[1].trim()
  ...
  const xsrfHeaderCookie = xsrfTokenAdminGet.header['set-cookie']
  const splitXsrfToken = xsrfHeaderCookie[0].split(';', 1)
  xsrfTokenAdmin = splitXsrfToken[0].split('=', 2)[1].trim()
})
...
describe('API /messages', () => {
  ...
  test('should not create a message xsrf_token was not sent.', async () => {
    const response = await request(app)
      .post('/messages')
      .send(newMessage)
